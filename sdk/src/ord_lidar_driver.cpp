...
[[nodiscard]] auto OrdlidarDriver::connect() -> bool
{
...

  {
    ...
    if (!serial_->open()) { return is_connected_; }
...
void OrdlidarDriver::uartDataFindInitInfo(unsigned char *data, int len)
{
  ...
  if (pos != 0) { cmd_buf_.erase(cmd_buf_.begin(), cmd_buf_.begin() + pos); }
...
bool OrdlidarDriver::uartDataHandle(unsigned char *data, int len)
{
  ...
  while (pos < (bin_buf_.size() - 2)) {
    ...
    if (pos != 0) {
      for (int i = 0; i < pos; i++) { bin_buf_.erase(bin_buf_.begin()); }
    }
    ...
    if (VERSION_MS200 == parsed_data_.version) {
      ...
      if (crc_check == pkg->crc8) {
        ...
        if (temp_data.vailtidy_point_num + parsed_data_.point_num < POINT_CIRCLE_MAX_SIZE) {
          ...
          if (parsed_data_.point_num < POINT_PKG_MAX_SIZE) {
            ...
            memcpy(&one_scan_data_.data[0],
              &temp_data.data[temp_data.vailtidy_point_num],
              sizeof(point_data_t) * parsed_data_.point_num);
          ...
          for (int i = 0; i < POINT_PER_PACK; i++) {
            if ((last_angle - temp_data.data[temp_data.vailtidy_point_num + i].angle) > 1) {
              ...
              memcpy(&temp_data.data[0],
                &temp_data.data[temp_data.vailtidy_point_num + i],
                sizeof(point_data_t) * (POINT_PER_PACK - i));
        ...
        for (uint32_t i = 0; i < sizeof(OradarLidarFrame); i++) { bin_buf_.erase(bin_buf_.begin()); }

        if (bin_buf_.size() < sizeof(OradarLidarFrame)) { break; }
      } else {
        parsed_data_.head_flag = HEAD_FLAG_NONE;
        parsed_data_.version = VERSION_NONE;
        /*only remove header,not all frame,because lidar data may contain head*/
        for (int i = 0; i < 2; i++) { bin_buf_.erase(bin_buf_.begin()); }
      }
    }
  }
  return true;
...
[[nodiscard]] auto OrdlidarDriver::write(unsigned char *data, int length) -> int
{
  ...
  if (ret < 0) printf("lidar write error \n");
